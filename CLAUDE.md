...
- **Your own trail re-smells you** (owner's request, `touchOwnTrail()` in `js/game.js`): stepping onto a *finished* trail of yours (within `TRAIL_STEP_ON` = 12px, before it expires) does exactly what stepping into a puddle does (`stepInPuddle`: 5s of walking smell + a new trail; a slightly quieter splash). **Cooldown `TRAIL_RESMELL_COOLDOWN` = 10s** (in `js/level.js`, `player.trailCd`, counts game time) from each trail re-smell; puddles ignore it. It only fires when the player is NOT already smelly (no top-up from trails), never from the trail still being laid, and never from the trail just finished (`player.justLeft`) until the player has stepped off it - without that the smell would restart instantly, because you are standing on the tip of the trail the moment your smell runs out. Standing on a trail when the cooldown ends counts (you are on it). Because the cooldown (10s) is longer than the smell (5s of walking), following your own old trail can never keep you permanently smelly.
...
- Hardcore's **high score stays visible** on the title screen (its button tag and a "High score" line): the furthest level reached, or "finished the game" once level 7 is cleared. It is recorded when a level is cleared (and on starting a Hardcore level, so a run that dies on level 1 still counts) and is never lowered. Level 1 has no monsters, so you cannot die there.

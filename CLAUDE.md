...
- **Monsters per level are EXACT and identical in every mode** (`ECHO_MONSTERS` / `SCENT_MONSTERS` in `js/level.js`, owner's schedule): echo 0,1,1,2,2,1,1 and scent 0,0,0,0,0,1,1 for levels 1-7. Levels 4-5 = 2 echo; levels 6-7 = 1 echo + 1 scent. The modes differ in speed, hearing, ripples and puddles - NOT in monster counts. (Levels past 7 use a placeholder formula only so debug/testing keeps working; they are not part of the game.)

...
**ECHO MAZE** on the title screen is drawn rather than typed ([`js/wordmark.js`](js/wordmark.js)): the page's own font at a heavy weight, laid out glyph by glyph with wide tracking on its own little canvas, fitted to whatever width it is given (and stacked onto two lines if one will not fit). It is lit in the **wall colour** — the blue a ripple paints a wall with, straight from the palette you have chosen — so the logo is painted with the same light as the maze, and changes with the palette.

Every few seconds a **ring of light expands from the middle of it**, brightening each letter as it passes through, with a faint wavefront behind: the game's own ripple, run through the logo. Additive glow, like every ripple hit. One sweep every 5.5 s, so the fastest thing on the screen changes about **0.2 times a second** — well under the game's three-a-second limit. When the title first appears, and whenever you come back to it from a level, the first sweep is a little brighter and quicker and comes with a soft whoosh (synthesised like everything else; silent until the first click has created the AudioContext, so it can never hold up **Begin**).

**Calm mode** — and your system's *reduce motion* setting — keep the loop but take the edge off it: a dimmer glow, a shallower breath, and no brighter, faster reveal. The logo is never removed. There is nothing to save and nothing to switch on: it costs about **0.01 ms a frame**, only runs while the title is actually on screen, and if a canvas cannot be had at all the `<h1>` falls back to styled text.

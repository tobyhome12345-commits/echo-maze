...
Every other screen is built out of the title screen's language, so the whole game reads as one thing. **Blue** is you and your interface and **green** is the way out, everywhere: the pause menu wears the title's panel, the caught screen wears it lit red, and level complete and victory wear it lit green. Key chips look the same pressed-metal way wherever a key is named — the pause menu, **Settings → Controls**, the title. Buttons, the settings tabs, the level grid, the cutscene list, the checkboxes and the volume sliders all take the same glow when you hover, focus or press them.
